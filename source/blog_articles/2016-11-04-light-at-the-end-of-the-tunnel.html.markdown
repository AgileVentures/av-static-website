...
* Poltergeist with PhantomJS - Stripe tests past, but redirects to success page not working - underlying permission fail?
...
I wasn't liking any of the options very much.  I thought I'd give stripe-ruby-mock a spin.  Working from [Thomas' CraftAcademy blog](https://medium.com/craft-academy/keeping-it-simple-3e7d9b186015#.4rc29xv6j), the [gem README](https://github.com/rebelidealist/stripe-ruby-mock/blob/master/README.md), and probably most importantly the associated [CraftAcademy repo](https://github.com/CraftAcademy/sf-online-august/) I got first a single Stripe acceptance test working, and then another.  The critical test was running in batch.  In batch still a large number of Stripe acceptance tests were failing, but importantly the ones using StripeRubyMock were not.  I had a lifeline, a light at the end of the tunnel.  If I could convert all the Stripe acceptance tests to StripeRubyMock I might have something where the entire test suite would go green.

I had created a custom stripe_javascript tag to ensure that our StripeRubyMock tests would not use PuffingBilly, while allowing the other tests to keep using it:
...
I was not without concern for some of the other changes.  App code had to be adjusted to accommodate testing with StripeRubyMock:
...
The charges controller would now ignore the incoming stripe token from the javascript redirect, and use a StripeRubyMock generated token in test mode.  It was pretty innocuous but a different code path would run in test, compared to development and production.  Another of those heuristics or guidelines.  Not the end of the world, but a warning flag.  Still this was starting to seem like the most hopeful route back to a green cucumber suite.  I also came away with a modified poltergeist driver:
...
```
based on the CraftAcademy code. Another key difference was that I know had to explicitly create our Premium plans using the StripeRubyMock as part of the setup for the cuke scenarios:
...
A card token from the Stripe test server would no longer work and the fix was using the stripe test helper to generate a card token.  I also had to modify the app code for updating cards, because we were now generating a different kind of error.  I added a `NoMethodError` to the update card error handling:
...
* ensuring development flow is comprehensible and manageable for developers on your team
* keep your full test suite green
* keep the running time of your test suite down to avoid delayed debug cycle hell

and various others.  None of these heuristics always trumps all the others.  As Kent Beck says, "it depends ..."  
