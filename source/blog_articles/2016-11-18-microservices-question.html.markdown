...
I'm still not sure on the microservices question.  Will breaking things up help us avoid a monolith, and follow the single responsibility principle of having one app for each separate function?  It's worked out well for Amazon, but then they're at a completely different scale.  Right now I feel like I'm maintaining three different monoliths (LocalSupport, WebSiteOne and AutoGraders) and it always feels precarious when adding a new element.  I guess it's ant colonies versus large vertebrates; single celled versus multi-cellular organisms.  There are pros and cons to each degree of organisational coupling.  As Kent Beck would say, it depends ... :-) 
